...
<div class="meinhaus-header">
    <div class="meinhuas-content">
        ...
        <p>{{ ucwords($data[0]->name) }}</p>

        <p style="font-weight: 400">{{ ucfirst($data[0]->locality) }} <br>{{ ucfirst($data[0]->city) }}, {{ ucfirst($data[0]->pincode) }} <br>{{ ucfirst($data[0]->state) }}</p>

        <p  style="font-weight: 400">{{ $data[0]->phone_no }}<br>{{ $data[0]->email }}</p>
...
<table class="table">
  ...
  <?php
        ...
        $user_invoice_total = DB::table('multiple_estimate_invoices_details')->where('invoice_id',$invoice_id)->sum('invoice_amount');
  ...
  <table class="table table-two">
    <tbody>
        <tr>
            <th>Project Total : :</th>
            <td>${{number_format($pro_total, 2)}}</td>
        </tr>
        <tr>
            <th>Current Invoice Amount :</th>
        <td>${{number_format($user_invoice_total,2)}}</td>
        </tr>
        <p class="description-box">
            <?php $check_pay_status = DB::table('multiple_estimate_invoices')->where('booking_id',$data[0]->booking_show_id)->first();?>
            @if($check_pay_status->payment_status == 0)
                <button><a href="javascript:void(0)">Unpaid</a></button>
            @elseif($check_pay_status->payment_status == 1)
